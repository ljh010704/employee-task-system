...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { data, error } = await client.from('collector_commands').update({ status: 'running', agent_id: agentId, started_at: new Date().toISOString() }).eq('id', id).eq('status', 'pending').select('id,store_id,command_type,status').maybeSingle();
