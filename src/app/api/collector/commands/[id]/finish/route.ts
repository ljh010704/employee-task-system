...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { data: command, error: findError } = await client.from('collector_commands').select('id,store_id').eq('id', id).maybeSingle();
    ...
    if (body.status === 'reauth_required') await client.from('store_configs').update({ status: 'reauth_required', last_error: body.error || 'Browser session requires login', updated_at: finishedAt }).eq('id', command.store_id);
