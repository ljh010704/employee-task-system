...
export default function StoresPage() {
  ...
  const [form, setForm] = useState({ store_code: '', store_name: '', browser_profile_id: '' });

  const load = async () => {
    setLoading(true);
    setError('');
    const [{ data: storeData, error: storeError }, { data: runData, error: runError }, { data: agentData, error: agentError }, { data: commandData, error: commandError }] = await Promise.all([
    ...
    if (storeError || runError || agentError || commandError) setError(storeError?.message || runError?.message || agentError?.message || commandError?.message || '店铺数据加载失败');
  ...
  const createStore = async (event: FormEvent) => {
    ...
    if (insertError) setError(insertError.message);
      ...
      setForm({ store_code: '', store_name: '', browser_profile_id: '' });
      await load();
    }
    setSaving(false);
  };
    ...
    <main className="min-h-screen bg-[#fbfbfa] text-stone-800 p-5 sm:p-8">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <section className="bg-white rounded-2xl border border-stone-200 p-5">
          ...
          <form onSubmit={createStore} className="grid grid-cols-1 md:grid-cols-4 gap-3 mt-4">
